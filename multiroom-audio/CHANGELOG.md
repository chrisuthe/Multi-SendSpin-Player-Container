...
## [5.3.0] - Output Delay Spec, Hi-Res Formats & Silent Card Recovery
...
### Changed
- **After upgrading, check any zone you had set a delay on.** Delays are migrated automatically: a zone set to play *earlier* keeps its timing exactly, since that is what the spec's output delay expresses. A zone set to play *later* has no equivalent under the spec and returns to no delay - if you were using one to hold a zone back, that zone was compensating for something the sync system should be handling, and is worth re-checking now
...
- Existing format preferences in `players.yaml` keep working untouched. A preference may now carry a bit depth (`flac-96000-24`) and a missing one is resolved from the device, while the new `"auto"` means the device's native best. The onboarding wizard is deliberately unchanged - players it creates get the default anchor, and format stays an advanced setting you change afterwards (#280)
- Dependency updates: System.IO.Ports 10.0.11, Microsoft.AspNetCore.SignalR 1.2.12, Microsoft.AspNetCore.OpenApi 8.0.30 and Microsoft.Extensions.Diagnostics.HealthChecks 10.0.11 (#275, #276, #277, #278)
